...
export function DeckEstimationDrill() {
  const settings = storage.settings();
  const [decks, setDecks] = useState(settings.decks), [resolution, setResolution] = useState<DeckResolution>(0.5), [lastDeck, setLastDeck] = useState(false), [landmarks, setLandmarks] = useState(true), [trayStyle, setTrayStyle] = useState<"green" | "red" | "smoke" | "photo">("green");
  ...
  const newTray = () => {
    if (trayStyle === "photo") {
      ...
      setPhoto(null);
      const exact = lastDeck ? Math.max(0.1, Math.random()) : Math.max(0.1, decks * (0.18 + Math.random() * 0.72));
      setRemaining(exact);
    }
  ...
  if (phase === "done" && result) return <SessionSummary session={result} onNew={() => setPhase("setup")} />;
  return <><Heading eyebrow="Visual drill" title="Deck Estimation" description="Read a perspective discard tray at full-, half-, or quarter-deck resolution. The fill always represents cards already dealt, not cards remaining." />
    {phase === "setup" ? <Panel className="max-w-4xl"><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><Select label="Shoe" value={decks} onChange={(e) => setDecks(+e.target.value)}>{(trayStyle === "photo" ? PHOTO_DECK_OPTIONS : [1, 2, 4, 6, 8]).map((x) => <option key={x} value={x}>{x} decks</option>)}</Select><Select label="Resolution" value={resolution} onChange={(e) => setResolution(+e.target.value as DeckResolution)}><option value="1">Full deck</option><option value="0.5">Half deck</option><option value="0.25">Quarter deck</option></Select><Select label="Tray style" value={trayStyle} onChange={(e) => setTrayStyle(e.target.value as typeof trayStyle)}><option value="green">Green acrylic</option><option value="red">Red acrylic</option><option value="smoke">Smoked acrylic</option><option value="photo">Photo (real trays)</option></Select><Select label="Feedback" value={feedbackMode} onChange={(e) => setFeedbackMode(e.target.value as typeof feedbackMode)}><option value="immediate">Immediate</option><option value="end">End of session</option></Select>{trayStyle === "photo" ? <p className="flex min-h-11 items-center rounded-xl bg-black/20 px-3 text-xs text-zinc-500">{PHOTO_UNIQUE_COUNT} real discard-tray photos ({DECK_ESTIMATION_PHOTOS.length} recorded rounds) across {PHOTO_DECK_OPTIONS.join(", ")}-deck shoes.</p> : <><label className="flex min-h-11 items-center gap-3 rounded-xl bg-black/20 px-3 text-sm"><input className="h-5 w-5 accent-emerald-500" type="checkbox" checked={landmarks} onChange={(e) => setLandmarks(e.target.checked)} />Tray landmarks</label><label className="flex min-h-11 items-center gap-3 rounded-xl bg-black/20 px-3 text-sm"><input className="h-5 w-5 accent-emerald-500" type="checkbox" checked={lastDeck} onChange={(e) => setLastDeck(e.target.checked)} />Last-deck precision</label></>}</div><Button className="mt-5 min-h-11" onClick={start}>Start {target} estimates</Button></Panel> : <div className="grid gap-5 lg:grid-cols-[1fr_18rem]"><Panel>{phase === "feedback" ? <div aria-live="polite" className="grid min-h-80 place-items-center text-center"><div><p className="text-2xl font-semibold">{message}</p><p className="mt-2 text-zinc-400">{(decks - remaining).toFixed(2)} decks are in the discard tray.</p><Button className="mt-5 min-h-11" onClick={newTray}>Next tray</Button></div></div> : <form className="mx-auto max-w-xl" onSubmit={(e) => { e.preventDefault(); submit(); }}>{trayStyle === "photo" && photo ? <div><img src={`/blackjack/deck-estimation/${photo.file}`} alt="Discard tray" className="mx-auto max-h-80 rounded-2xl border border-white/15 bg-black/40 object-contain shadow-inner" /><p className="mt-2 text-center text-xs text-zinc-500">{decks}-deck shoe</p></div> : <TrayVisual totalDecks={decks} remainingDecks={remaining} style={trayStyle === "photo" ? "green" : trayStyle} landmarks={landmarks} />}<label className="mx-auto mt-6 block max-w-xs text-center text-sm text-zinc-400">Decks remaining<input autoFocus inputMode="decimal" className={`${inputClass} mt-2`} value={answer} onChange={(e) => setAnswer(e.target.value)} /></label><Button className="mx-auto mt-4 hidden min-h-11 sm:block">Check estimate</Button><NumericPad decimal value={answer} onChange={setAnswer} onSubmit={submit} /></form>}</Panel><div className="space-y-3"><Metric label="Estimate" value={`${question + 1} / ${target}`} /><Metric label="Accuracy" value={`${question ? Math.round(correct / question * 100) : 0}%`} /><Metric label="Mean error" value={`${errors.length ? (errors.reduce((a, b) => a + b, 0) / errors.length).toFixed(2) : "0.00"} decks`} /></div></div>}
  </>;
}
