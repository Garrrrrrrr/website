...
export function getBasicStrategyDecision({playerCards,dealerUpcard,rules}:{playerCards:Card[];dealerUpcard:Card;rules:BlackjackRules}):Decision {
  const total=calculateHandValue(playerCards), up=upValue(dealerUpcard), soft=isSoft(playerCards);
  let action:Action="H";
  if(rules.lateSurrender && !soft && playerCards.length===2 && ((total===16&&[9,10,11].includes(up))||(total===15&&up===10))) action="R";
  else if(isPair(playerCards)) { const r=playerCards[0].rank; if(["A","8"].includes(r)) action="P"; else if(["10","J","Q","K"].includes(r)) action="S"; else if(r==="9") action=[2,3,4,5,6,8,9].includes(up)?"P":"S"; else if(r==="7") action=up<=7?"P":"H"; else if(r==="6") action=up<=6?"P":"H"; else if(r==="4") action=rules.doubleAfterSplit&&[5,6].includes(up)?"P":"H"; else if(["2","3"].includes(r)) action=up<=7?"P":"H"; else if(r==="5") action=up<=9?"D":"H";
  ...
  const kind=isPair(playerCards)?"pair":soft?"soft hand":"hard hand";
